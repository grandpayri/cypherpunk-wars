...
**Testnet only, do not suggest mainnet or real funds.** `design-bible.md` section 08 ("Trust Model") documents exactly what's cryptographically enforced today (per-transaction Phish yield, since 2026-07-11 — the payload's declared yield is bound on-chain to the ZK proof's own output via `OpTxPayloadSubstr`) versus what still relies on trusting the official client (a player's *cumulative* $PUNKW total — no on-chain mechanism yet ties it to a genuine prior-transaction history) and what decays over time (payload readability, ~3 days on a default/non-archival node). Read that section before making any claim about this project's trust guarantees, and do not treat it as ready for mainnet or real value until it says otherwise.
