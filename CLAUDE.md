...
**Testnet only, do not suggest mainnet or real funds.** `design-bible.md` section 07 ("Trust Model") documents exactly what's cryptographically enforced today (per-transaction Phish yield, since 2026-07-11 — the payload's declared yield is bound on-chain to the ZK proof's own output via `OpTxPayloadSubstr`) versus what still relies on trusting the official client (a player's *cumulative* $PUNKW total — no on-chain mechanism yet ties it to a genuine prior-transaction history) and what decays over time (payload readability, ~3 days on a default/non-archival node). Read that section before making any claim about this project's trust guarantees, and do not treat it as ready for mainnet or real value until it says otherwise.
...
Flow: `index.html` (login/detect saved session) → `forge.html` (generate new 24-word identity, **and** the rest of first-time onboarding in the same continuous flow — starter faucet grants, claim genesis 1,000 $PUNKW, fund the Gameplay Vault via a real Buy Turns purchase; formerly split across a separate `initialize.html`, merged 2026-07-11) **or** `import.html` (recover from existing seed) → `bunker.html` (main command-center loop, e.g. `executePhish()` in `bunker-logic.js`).
...
- All chain interaction lives in `kaspa-client.js`: `getAddressBalance()`, `sendTaggedTransaction()` (Genesis), `spendZkVerifiedPhish()`/`submitPhishAnchor()` (Phish), `spendBuildSector()` (Build Sector — since 2026-07-11, spends 350 $PUNKW to grow Sectors by 1, ZK-verified via `buildSector.circom`), all three sharing one covenant/address (`buildZkPhishRedeemScript`/`deriveZkPhishAddress` — the names predate Build Sector and haven't been renamed, see that function's own comment) via a nested `OpIf`/`OpElse` (outer: claim-family vs the cheap anchor branch; inner: Phish vs Build — see the "ZK-verified Phish covenant" section comment), `requestFaucetGrant()`, `buyTurns()`, `fetchAddressTransactionList()`/`fetchTransactionPayloadBytes()` (CPW History's read side). `spendFromRestrictedWallet()`/`deriveRestrictedWalletAddress()` still exist but are now only used by `send.html`'s covenant-rejection demo, not live gameplay.
- The faucet covenant itself has no memory and will let any address claim repeatedly (rate-limited by the grant cap, not sybil-resistant — see the Phase 7 notes in the plan doc). `hasReceivedFaucetGrant(address)` in `kaspa-client.js` is a client-side-only courtesy check (queries the address's own tx history via REST for a prior incoming transfer from `FAUCET_VAULT_ADDRESS`) that `forge.html` calls once before its whole starter-grant sequence (not per-grant — both starter grants land at the same Plain Wallet address now, so a per-grant check would see the first grant and incorrectly skip the second) so *our own UI* won't double-grant — not real sybil resistance, just stops the common case.

## State & persistence

- The only persisted client state is `localStorage['bunker_id']` (the derived Kaspa address), used both as the "is there a saved session" check on `index.html` and as the identity key everywhere else, plus per-identity `localStorage['punkw_balance_<address>']`/`localStorage['turn_count_<address>']`/`localStorage['sector_count_<address>']` caches (see `shared-sidebar.js`'s `getPunkwBalance`/`setPunkwBalance`/`getTurnCount`/`setTurnCount`/`getSectorCount`/`setSectorCount`) kept in sync with the full game-state snapshot now genuinely written into every Genesis/Phish/Build Sector payload on-chain (confirmed live, decoded back out via `history.html`).
- Sectors are a real, buildable mechanic (Phase 6, live since 2026-07-11): `bunker.html`'s Build Sector action spends 350 $PUNKW (`COST_PER_SECTOR` in `zk-spike/circuits/buildSector.circom`) to grow Sectors by exactly 1, ZK-verified the same way as Phish and feeding back into Phish's own yield formula via `sectorBonus`. There is no GWh or Cycles system — those were an earlier, incorrect design pass and have been removed from both the docs and the UI.
...
- **$PUNKW (War Chest)** — earned by Phishing. Every Genesis/Phish/Build Sector transaction writes the operator's entire current game state (a fixed-width, versioned 42-byte struct — $PUNKW, sectors, node specialization, research tier, turn count, season, unit/item counts) into that transaction's on-chain payload (`encodeGameStateSnapshot`/`decodeGameStateSnapshot` in `kaspa-client.js`), confirmed live and decodable via `history.html`. `localStorage`'s punkw/turn-count/sector-count cache (see State & persistence above) remains the sidebar's fast-path source of truth (no indexer for "current state" yet — see Phase 5 in the plan doc), kept in sync with what's on-chain.
- **Sectors** — live (Phase 6): built via `bunker.html`'s Build Sector action, which spends 350 $PUNKW per Sector (ZK-verified, `zk-spike/circuits/buildSector.circom`) and increases Phishing efficiency through the yield formula's `sectorBonus` term.
