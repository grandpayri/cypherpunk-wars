...
var require_buffer = __commonJS({
  "node_modules/buffer/index.js"(exports) {
    ...
    Buffer3.prototype.write = function write2(string2, offset, length, encoding) {
      if (offset === void 0) {
        encoding = "utf8";
        length = this.length;
        offset = 0;
...
// node-globals-shim.js
var import_buffer, import_browser;
var init_node_globals_shim = __esm({
  "node-globals-shim.js"() {
...
async function plonk16Prove(zkeyFileName, witnessFileName, logger, options) {
  ...
  if (logger) logger.debug("\xB7\xB7\xB7 Reading Sigma polynomials ");
  ...
  if (logger) logger.debug("\xB7\xB7\xB7 Reading Sigma evaluations");
  ...
  async function calculateAdditions() {
    if (logger) logger.debug("\xB7\xB7\xB7 Computing additions");
  ...
  async function computeWirePolynomials() {
    if (logger) logger.debug("\xB7\xB7\xB7 Reading data from zkey file");
  ...
  async function round2() {
    ...
    if (logger) logger.debug("\xB7\xB7\xB7 challenges.beta: " + Fr.toString(challenges.beta, 16));
    transcript.reset();
    transcript.addScalar(challenges.beta);
    challenges.gamma = transcript.getChallenge();
    if (logger) logger.debug("\xB7\xB7\xB7 challenges.gamma: " + Fr.toString(challenges.gamma, 16));
  ...
  async function computeZ() {
    if (logger) logger.debug("\xB7\xB7\xB7 Computing Z evaluations");
    ...
    if (logger) logger.debug("\xB7\xB7\xB7 Computing Z ifft");
    polynomials.Z = await Polynomial.fromEvaluations(buffers.Z, curve2, logger);
    if (logger) logger.debug("\xB7\xB7\xB7 Computing Z fft");
  ...
  async function round3() {
    ...
    if (logger) logger.debug("\xB7\xB7\xB7 challenges.alpha: " + Fr.toString(challenges.alpha, 16));
  ...
  async function computeT() {
    ...
    if (logger) logger.debug("\xB7\xB7\xB7 Computing T evaluations");
    let w = Fr.one;
    ...
    if (logger) logger.debug("\xB7\xB7\xB7 Computing T1, T2, T3 polynomials");
  ...
  async function round4() {
    ...
    if (logger) logger.debug("\xB7\xB7\xB7 challenges.xi: " + Fr.toString(challenges.xi, 16));
  ...
  async function round5() {
    ...
    if (logger) logger.debug("\xB7\xB7\xB7 challenges.v: " + Fr.toString(challenges.v[1], 16));
    for (let i = 2; i < 6; i++) {
...
async function fflonkSetup(r1csFilename, ptauFilename, zkeyFilename, logger) {
  ...
  async function writeZkeyFile() {
    ...
    if (logger) logger.info(`\xB7\xB7\xB7 Writing Section ${HEADER_ZKEY_SECTION}. Zkey Header`);
    await writeZkeyHeader(fdZKey);
    if (logger) logger.info(`\xB7\xB7\xB7 Writing Section ${ZKEY_FF_ADDITIONS_SECTION}. Additions`);
    await writeAdditions(fdZKey);
    if (globalThis.gc) globalThis.gc();
    if (logger) logger.info(`\xB7\xB7\xB7 Writing Section ${ZKEY_FF_A_MAP_SECTION}. A Map`);
    await writeWitnessMap(fdZKey, ZKEY_FF_A_MAP_SECTION, 0, "A map");
    if (globalThis.gc) globalThis.gc();
    if (logger) logger.info(`\xB7\xB7\xB7 Writing Section ${ZKEY_FF_B_MAP_SECTION}. B Map`);
    await writeWitnessMap(fdZKey, ZKEY_FF_B_MAP_SECTION, 1, "B map");
    if (globalThis.gc) globalThis.gc();
    if (logger) logger.info(`\xB7\xB7\xB7 Writing Section ${ZKEY_FF_C_MAP_SECTION}. C Map`);
    await writeWitnessMap(fdZKey, ZKEY_FF_C_MAP_SECTION, 2, "C map");
    if (globalThis.gc) globalThis.gc();
    if (logger) logger.info(`\xB7\xB7\xB7 Writing Section ${ZKEY_FF_QL_SECTION}. QL`);
    await writeQMap(fdZKey, ZKEY_FF_QL_SECTION, 3, "QL");
    if (globalThis.gc) globalThis.gc();
    if (logger) logger.info(`\xB7\xB7\xB7 Writing Section ${ZKEY_FF_QR_SECTION}. QR`);
    await writeQMap(fdZKey, ZKEY_FF_QR_SECTION, 4, "QR");
    if (globalThis.gc) globalThis.gc();
    if (logger) logger.info(`\xB7\xB7\xB7 Writing Section ${ZKEY_FF_QM_SECTION}. QM`);
    await writeQMap(fdZKey, ZKEY_FF_QM_SECTION, 5, "QM");
    if (globalThis.gc) globalThis.gc();
    if (logger) logger.info(`\xB7\xB7\xB7 Writing Section ${ZKEY_FF_QO_SECTION}. QO`);
    await writeQMap(fdZKey, ZKEY_FF_QO_SECTION, 6, "QO");
    if (globalThis.gc) globalThis.gc();
    if (logger) logger.info(`\xB7\xB7\xB7 Writing Section ${ZKEY_FF_QC_SECTION}. QC`);
    await writeQMap(fdZKey, ZKEY_FF_QC_SECTION, 7, "QC");
    if (globalThis.gc) globalThis.gc();
    if (logger) logger.info(`\xB7\xB7\xB7 Writing Sections ${ZKEY_FF_SIGMA1_SECTION},${ZKEY_FF_SIGMA2_SECTION},${ZKEY_FF_SIGMA3_SECTION}. Sigma1, Sigma2 & Sigma 3`);
    await writeSigma(fdZKey);
    if (globalThis.gc) globalThis.gc();
    if (logger) logger.info(`\xB7\xB7\xB7 Writing Section ${ZKEY_FF_LAGRANGE_SECTION}. Lagrange Polynomials`);
    await writeLagrangePolynomials(fdZKey);
    if (globalThis.gc) globalThis.gc();
    if (logger) logger.info(`\xB7\xB7\xB7 Writing Section ${ZKEY_FF_PTAU_SECTION}. Powers of Tau`);
    await writePtau(fdZKey);
    if (globalThis.gc) globalThis.gc();
    if (logger) logger.info(`\xB7\xB7\xB7 Writing Section ${ZKEY_FF_C0_SECTION}. C0`);
    await writeC0(fdZKey);
    if (globalThis.gc) globalThis.gc();
    if (logger) logger.info(`\xB7\xB7\xB7 Writing Section ${ZKEY_FF_HEADER_SECTION}. FFlonk Header`);
...
async function fflonkProve(zkeyFileName, witnessFileName, logger, options) {
  ...
  if (logger) logger.info("\xB7\xB7\xB7 Reading Sigma polynomials ");
  polynomials.Sigma1 = new Polynomial(new BigBuffer(sDomain), curve2, logger);
  polynomials.Sigma2 = new Polynomial(new BigBuffer(sDomain), curve2, logger);
  polynomials.Sigma3 = new Polynomial(new BigBuffer(sDomain), curve2, logger);
  ...
  if (logger) logger.info("\xB7\xB7\xB7 Reading Sigma evaluations");
  evaluations.Sigma1 = new Evaluations(new BigBuffer(sDomain * 4), curve2, logger);
  evaluations.Sigma2 = new Evaluations(new BigBuffer(sDomain * 4), curve2, logger);
  evaluations.Sigma3 = new Evaluations(new BigBuffer(sDomain * 4), curve2, logger);
  ...
  async function calculateAdditions() {
    if (logger) logger.info("\xB7\xB7\xB7 Computing additions");
  ...
  async function round1() {
    ...
    return 0;
    async function computeWirePolynomials() {
      if (logger) logger.info("\xB7\xB7\xB7 Reading data from zkey file");
      buffers.A = new BigBuffer(sDomain);
      buffers.B = new BigBuffer(sDomain);
      buffers.C = new BigBuffer(sDomain);
      ...
      buffers.B = await Fr.batchToMontgomery(buffers.B);
      buffers.C = await Fr.batchToMontgomery(buffers.C);
      if (logger) logger.info("\xB7\xB7\xB7 Computing A ifft");
      polynomials.A = await Polynomial.fromEvaluations(buffers.A, curve2, logger);
      if (logger) logger.info("\xB7\xB7\xB7 Computing B ifft");
      polynomials.B = await Polynomial.fromEvaluations(buffers.B, curve2, logger);
      if (logger) logger.info("\xB7\xB7\xB7 Computing C ifft");
      polynomials.C = await Polynomial.fromEvaluations(buffers.C, curve2, logger);
      if (logger) logger.info("\xB7\xB7\xB7 Computing A fft");
      evaluations.A = await Evaluations.fromPolynomial(polynomials.A, 4, curve2, logger);
      if (logger) logger.info("\xB7\xB7\xB7 Computing B fft");
      evaluations.B = await Evaluations.fromPolynomial(polynomials.B, 4, curve2, logger);
      if (logger) logger.info("\xB7\xB7\xB7 Computing C fft");
      evaluations.C = await Evaluations.fromPolynomial(polynomials.C, 4, curve2, logger);
    ...
    async function computeT0() {
      if (logger) logger.info(`\xB7\xB7\xB7 Reading sections ${ZKEY_FF_QL_SECTION}, ${ZKEY_FF_QR_SECTION}, ${ZKEY_FF_QM_SECTION}, ${ZKEY_FF_QO_SECTION}, ${ZKEY_FF_QC_SECTION}. Q selectors`);
      evaluations.QL = new Evaluations(new BigBuffer(sDomain * 4), curve2, logger);
      evaluations.QR = new Evaluations(new BigBuffer(sDomain * 4), curve2, logger);
      evaluations.QM = new Evaluations(new BigBuffer(sDomain * 4), curve2, logger);
      evaluations.QO = new Evaluations(new BigBuffer(sDomain * 4), curve2, logger);
      ...
      if (logger) logger.info("\xB7\xB7\xB7 Computing T0 evaluations");
      for (let i = 0; i < zkey2.domainSize * 4; i++) {
      ...
      if (logger) logger.info("\xB7\xB7\xB7 Computing T0 ifft");
      ...
      if (logger) logger.info("\xB7\xB7\xB7 Computing T0 / ZH");
  ...
  async function round2() {
    ...
    challenges.beta = transcript.getChallenge();
    if (logger) logger.info("\xB7\xB7\xB7 challenges.beta: " + Fr.toString(challenges.beta));
    transcript.reset();
    transcript.addScalar(challenges.beta);
    challenges.gamma = transcript.getChallenge();
    if (logger) logger.info("\xB7\xB7\xB7 challenges.gamma: " + Fr.toString(challenges.gamma));
    ...
    return 0;
    async function computeZ() {
      if (logger) logger.info("\xB7\xB7\xB7 Computing Z evaluations");
      let numArr = new BigBuffer(sDomain);
      let denArr = new BigBuffer(sDomain);
      numArr.set(Fr.one, 0);
      denArr.set(Fr.one, 0);
      ...
      if (logger) logger.info("\xB7\xB7\xB7 Computing Z ifft");
      polynomials.Z = await Polynomial.fromEvaluations(buffers.Z, curve2, logger);
      if (logger) logger.info("\xB7\xB7\xB7 Computing Z fft");
      evaluations.Z = await Evaluations.fromPolynomial(polynomials.Z, 4, curve2, logger);
      polynomials.Z.blindCoefficients([challenges.b[9], challenges.b[8], challenges.b[7]]);
      if (polynomials.Z.degree() >= zkey2.domainSize + 3) {
        throw new Error("Z Polynomial is not well calculated");
    ...
    async function computeT1() {
      if (logger) logger.info("\xB7\xB7\xB7 Computing T1 evaluations");
      ...
      if (logger) logger.info("\xB7\xB7\xB7 Computing T1 ifft");
      ...
      if (logger) logger.info("\xB7\xB7\xB7 Computing T1z ifft");
    ...
    async function computeT2() {
      if (logger) logger.info("\xB7\xB7\xB7 Computing T2 evaluations");
      ...
      if (logger) logger.info("\xB7\xB7\xB7 Computing T2 ifft");
      polynomials.T2 = await Polynomial.fromEvaluations(buffers.T2, curve2, logger);
      if (logger) logger.info("\xB7\xB7\xB7 Computing T2 / ZH");
      polynomials.T2.divByZerofier(zkey2.domainSize, Fr.one);
      if (logger) logger.info("\xB7\xB7\xB7 Computing T2z ifft");
  ...
  async function round3() {
    ...
    if (logger) logger.info("\xB7\xB7\xB7 challenges.xi: " + Fr.toString(challenges.xi));
    polynomials.QL = new Polynomial(new BigBuffer(sDomain), curve2, logger);
    polynomials.QR = new Polynomial(new BigBuffer(sDomain), curve2, logger);
    polynomials.QM = new Polynomial(new BigBuffer(sDomain), curve2, logger);
    polynomials.QO = new Polynomial(new BigBuffer(sDomain), curve2, logger);
    ...
    if (logger) logger.info("\xB7\xB7\xB7 Computing evaluations");
  ...
  async function round4() {
    ...
    challenges.alpha = transcript.getChallenge();
    if (logger) logger.info("\xB7\xB7\xB7 challenges.alpha: " + Fr.toString(challenges.alpha));
    ...
    async function computeF2() {
      if (logger) logger.info("\xB7\xB7\xB7 Computing F polynomial");
  ...
  async function round5() {
    ...
    if (logger) logger.info("\xB7\xB7\xB7 challenges.y: " + Fr.toString(challenges.y));
    ...
    async function computeL() {
      if (logger) logger.info("\xB7\xB7\xB7 Computing L polynomial");
...
function computeR0(proof, challenges, roots, curve2, logger) {
  ...
  if (logger) logger.info("\xB7\xB7\xB7 Computing r0(y)");
  let res = Fr.zero;
  for (let i = 0; i < 8; i++) {
    let coefValues = [];
...
function computeR1(proof, challenges, roots, pi, curve2, logger) {
  ...
  if (logger) logger.info("\xB7\xB7\xB7 Computing T0(xi)");
  ...
  if (logger) logger.info("\xB7\xB7\xB7 Computing C1(h_1\u03C9_4^i) values");
  let res = Fr.zero;
  for (let i = 0; i < 4; i++) {
...
function computeR2(proof, challenges, roots, lagrange1, vk, curve2, logger) {
  ...
  if (logger) logger.info("\xB7\xB7\xB7 Computing T1(xi)");
  let t1 = Fr.sub(proof.evaluations.z, Fr.one);
  t1 = Fr.mul(t1, lagrange1);
  t1 = Fr.mul(t1, challenges.invzh);
  if (logger) logger.info("\xB7\xB7\xB7 Computing T2(xi)");
  const betaxi = Fr.mul(challenges.beta, challenges.xi);
  ...
  if (logger) logger.info("\xB7\xB7\xB7 Computing C2(h_2\u03C9_3^i) values");
  let res = Fr.zero;
  for (let i = 0; i < 3; i++) {
  ...
  if (logger) logger.info("\xB7\xB7\xB7 Computing C2(h_3\u03C9_3^i) values");
  for (let i = 0; i < 3; i++) {
